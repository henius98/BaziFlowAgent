...
pub async fn perform_bazi_analysis(
    ...
    let (birth_date, birth_hour, birth_minute, gender, location, location_status) =
    ...
    let structured_data = match crate::services::bazi_service::prepare_bazi_data(&state, params)
        .await
    {
        Ok(data) => data,
        Err(e) => {
            ...
            let _ = bot
                .send_message(
                    chat_id,
                    "❌ Error fetching Bazi chart from API. Please try again later.".to_string(),
                )
                .await;
            return Ok(());
        }
    ...
    crate::services::bazi_service::build_and_save_bazi_html(
        &state,
        user_id,
        &username,
        &structured_data,
    ...
    match crate::services::bazi_service::core_bazi_analysis(
        &state,
        user_id,
        &structured_data,
        llm_model,
    ...
    {
        Ok(receiver) => {
            let reading = super::helpers::stream_to_telegram(
                &bot,
                chat_id,
                "🔮 Generating bazi analysis...",
                receiver,
            )
            .await;
            if !reading.is_empty() {
                ...
                tokio::spawn(async move {
                    if let Ok(summary) = crate::services::bazi_service::generate_bazi_summary(
                        &state_clone,
                        user_id,
                        &reading,
                        llm_model,
                    ...
                    .await
                    {
                        repos::save_user_bazi_summary(&state_clone.db_pool, user_id, &summary)
                            .await;
                    } else {
        ...
        Err(e) => {
            ...
            let _ = bot
                .send_message(
                    chat_id,
                    "❌ Error generating Bazi analysis from AI. Please try again later."
                        .to_string(),
                )
                .await;
        }
...
pub async fn display_user_profile(bot: Bot, chat_id: ChatId, user_id: u64) -> ResponseResult<()> {
    ...
    match serde_json::from_str::<paipan::StructuredBazi>(user_bazi_four_pillars_raw) {
        Ok(structured) => {
            let gender_str = if structured.info.gender == "男,乾造" {
                "男 (Male)"
            } else {
                "女 (Female)"
            };
            ...
            for p in &structured.pillars {
                ...
                let branch_stars = p
                    ...
                    .iter()
                    .map(|(stem, star)| format!("{}/{}", stem, star))
                    .collect::<Vec<_>>()
                    .join(", ");
            ...
            let chart_url =
                match crate::services::bazi_service::get_bazi_chart_url(&state, user_id).await {
                    Ok(url) => url,
                    Err(e) => {
                        error!("Failed to generate bazi chart URL: {}", e);
                        format!(
            ...
            let llm_model_str = user_profile
                .llm_model
                .map(|m| m.as_str())
                .unwrap_or("Not set (未设置)");
            let schedule_str = user_profile
                .schedule
                .as_deref()
                .unwrap_or("Not set (未设置)");
            ...
            bot.send_message(chat_id, settings_msg)
                .parse_mode(teloxide::types::ParseMode::Html)
                .await?;
        ...
        Err(e) => {
            bot.send_message(
                chat_id,
                format!("❌ Error reading your Bazi profile data: {}", e),
            )
            .await?;
        }
    }
...
pub async fn process_date_selection(
    bot: &Bot,
    q: &CallbackQuery,
    formatted_date: &str,
    request_type: &str,
    ...
    if let Some(msg) = &q.message {
        ...
        let bazi_four_pillars = user_profile
            ...
            .as_deref()
            .and_then(|raw| {
                serde_json::from_str::<crate::services::paipan::models::StructuredBazi>(raw).ok()
            })
            .map(|b| b.to_string());
        ...
        let almanac_data = match crate::services::almanac::fetch_and_format_almanac(
            &state.http_client,
            formatted_date,
        )
        .await
        {
            ...
            Err(e) => {
                ...
                let _ = bot
                    .send_message(chat_id, format!("Error fetching almanac data: {}", e))
                    .await;
                return Ok(());
            }
        };
        ...
        let _ = bot
            .send_message(
                chat_id,
                format!("📅 【{} 黄历信息】\n\n{}", formatted_date, almanac_data),
            )
            .await;

        match crate::services::almanac::analysis_date_fortune(
            crate::services::almanac::DateFortuneRequest {
                target_date: formatted_date,
                almanac_data: &almanac_data,
                bazi_four_pillars,
                bazi_summary: user_profile
                    .bazi_summary
                    .as_deref()
        ...
        .await
        {
            Ok(crate::models::LlmResponse::Stream(receiver)) => {
                let result_text =
                    super::helpers::stream_to_telegram(bot, chat_id, response_prefix, receiver)
                        .await;
            ...
            Ok(_) => {
                ...
                let _ = bot
                    .send_message(chat_id, "Internal error: unexpected response type")
                    .await;
            }
            Err(e) => {
                error!("Error: {}", e);
                let _ = bot
                    .send_message(chat_id, format!("Error generating reading: {}", e))
                    .await;
            }
        }
    }
...
pub async fn process_pick_selection(
    bot: Bot,
    chat_id: ChatId,
    user_id: u64,
    ...
    if start_date.is_empty() || end_date.is_empty() || activity.is_empty() {
        let _ = bot
            .send_message(
                chat_id,
                "⚠️ Pick selection context is missing. Please restart the process with /pick.",
            )
            .await;
        return Ok(());
    }
    ...
    match crate::services::almanac::analysis_pick_selection(
        crate::services::almanac::PickSelectionRequest {
    ...
    .await
    {
        Ok(crate::models::LlmResponse::Stream(receiver)) => {
            let result_text =
                super::helpers::stream_to_telegram(&bot, chat_id, &placeholder_text, receiver)
                    .await;
