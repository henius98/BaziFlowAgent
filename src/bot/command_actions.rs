...
pub async fn perform_bazi_analysis(bot: Bot, chat_id: ChatId, user_id: u64, username: String, message_id: Option<MessageId>) -> ResponseResult<()> {
  ...
  let profile_state = match state.user_contexts.get_mut(&user_id) {
    Some(mut ctx) => {
      let profile_state = ctx.profile_state.clone();
  ...
  let (birth_date, gender) = match (profile_state.birthdate, profile_state.gender) {
    (Some(date), Some(gender)) => (date, gender),
    _ => {
      error!("Incomplete profile state for user_id: {}", user_id);
      let msg = "⚠️ Required profile data is incomplete. Please start again with /new.";
      if let Some(msg_id) = message_id {
  ...
  let birth_time = match (profile_state.hour, profile_state.minute) {
    (Some(hour), Some(minute)) => match chrono::NaiveTime::from_hms_opt(hour as u32, minute as u32, 0) {
      Some(time) => Some(time),
      None => {
        error!("Invalid profile time for user_id: {}", user_id);
        let msg = "⚠️ The selected birth time is invalid. Please start again with /new.";
        if let Some(msg_id) = message_id {
          let _ = bot.edit_message_text(chat_id, msg_id, msg).await;
        } else {
          let _ = bot.send_message(chat_id, msg).await;
    ...
    (None, None) => None,
    _ => {
      error!("Partially selected profile time for user_id: {}", user_id);
      let msg = "⚠️ Birth time is incomplete. Please start again with /new.";
      if let Some(msg_id) = message_id {
        let _ = bot.edit_message_text(chat_id, msg_id, msg).await;
      } else {
        let _ = bot.send_message(chat_id, msg).await;
  ...
  let location = profile_state.location;
  let location_status = if let Some(city) = &location { format!("✅ City selected: {}", city) } else { "⏩ Birth location skipped; no longitude correction will be applied.".to_string() };
  let time_status = birth_time
    .map_or_else(|| "⏩ Birth time skipped; 12:00 will be used only as a reference, so the hour pillar is uncertain.".to_string(), |time| format!("✅ Time received: {}", time.format("%H:%M")));

  let status_text = format!("{}\n{}\n\n📅 Date: {}\n⌛ Calculating your Bazi chart...", location_status, time_status, birth_date);
  ...
  if let Err(e) = state.chat_cache_writer.clear(user_id).await {
    error!("Failed to clear chat cache for user {}: {}", user_id, e);
  }
  if let Some(msg_id) = message_id {
  ...
  let params = crate::services::bazi_service::BaziDataParams { user_id, username: &username, birth_date: &birth_date, birth_time, gender, location };
  ...
  let _ = bot.send_message(chat_id, format!("📊 <b>Bazi Chart Diagram</b>\n\nView your chart here:\n{}", chart_url)).parse_mode(teloxide::types::ParseMode::Html).await;
  ...
  match crate::services::bazi_service::core_bazi_analysis(&state, user_id, &structured_data, llm_model).await {
    Ok(receiver) => {
      ...
      if !reading.is_empty() {
        ...
        if let Err(e) = state.chat_cache_writer.save(user_id, "assistant", &reading) {
          error!("Failed to cache assistant response for user {}: {}", user_id, e);
        }

        // Generate bazi summary before releasing ownership of this profile.
        let state_clone = state.clone();
        let structured_bazi = structured_data.to_string();
        async move {
          if let Ok(summary) = crate::services::bazi_service::generate_bazi_summary(&state_clone, user_id, &structured_bazi, &reading, llm_model).await {
...
pub async fn display_user_profile(bot: Bot, chat_id: ChatId, user_id: u64) -> ResponseResult<()> {
  ...
  let structured = match crate::utils::parse_user_bazi(user_profile.bazi_four_pillars.as_deref()) {
    Ok(s) => s,
    Err(_) => {
      bot.send_message(chat_id, "⚠️ You haven't set your birthdate yet or the data is invalid! Please use /new to set your birthdate first to get accurate analysis.").await?;
      return Ok(());
    }
  };
  {
    let gender_str = if structured.info.gender == "男,乾造" { "男 (Male)" } else { "女 (Female)" };
    let solar_str = if structured.info.birth_time_known { structured.info.solar_date.clone() } else { format!("{} (12:00 reference; birth time unknown)", structured.info.solar_date) };
    let lunar_str = &structured.info.lunisolar_date;
    let birth_location_str =
      if structured.info.birth_location == "未知" { "Unknown / skipped".to_string() } else { structured.info.birth_location.replace('&', "&amp;").replace('<', "&lt;").replace('>', "&gt;") };
    ...
    let profile_msg = format!(
      "👤 <b>Your Bazi Profile (个人八字档案)</b>\n\n\
                ...
                • <b>Birth Location (出生地):</b> {birth_location_str}\n\
...
pub async fn process_date_selection(bot: &Bot, q: &CallbackQuery, formatted_date: &str, request_type: crate::models::LlmRequestType, response_prefix: &str) -> ResponseResult<()> {
  let state = crate::models::get_state();
  let user = &q.from;
  ...
  if let Some(msg) = &q.message {
    ...
    let user_profile = repos::get_user_profile(&state.db_pool, user_id).await;
    let bazi_four_pillars = user_profile.bazi_four_pillars.as_deref().and_then(|raw| serde_json::from_str::<crate::services::paipan::models::StructuredBazi>(raw).ok()).map(|b| b.to_string());
    let Some(bazi_four_pillars) = bazi_four_pillars.as_deref() else {
      let _ = bot.send_message(chat_id, "⚠️ You haven't set your birthdate yet! Please use /new to set your birthdate first to get accurate readings.").await;
      return Ok(());
    };
    ...
    .await
    {
      Ok(crate::models::LlmResponse::Stream(receiver)) => {
        ...
        if let Err(e) = state.chat_cache_writer.save(user_id, "assistant", &result_text) {
          error!("Failed to cache assistant response for user {}: {}", user_id, e);
        }
      }
      Ok(_) => {
...
pub async fn process_pick_selection(bot: Bot, chat_id: ChatId, user_id: u64, target: Option<MessageTarget>) -> ResponseResult<()> {
  ...
  let (start_date, end_date, activity) = match ctx {
    Some(c) => (c.pick_state.start_date.clone().unwrap_or_default(), c.pick_state.end_date.clone().unwrap_or_default(), c.pick_state.activity.clone().unwrap_or_default()),
    None => return Ok(()),
  };
  ...
  match &target {
    Some(MessageTarget::Edit(m_id)) => {
      let _ = bot.edit_message_text(chat_id, *m_id, format!("⏳ Analyzing dates from {} to {} for activity: '{}'...\nThis may take a minute.", start_date, end_date, activity)).await;
    }
    Some(MessageTarget::Reply(m_id)) => {
      let _ = bot
        .send_message(chat_id, format!("⏳ Analyzing dates from {} to {} for activity: '{}'...\nThis may take a minute.", start_date, end_date, activity))
  ...
  match crate::services::almanac::analysis_pick_selection(crate::services::almanac::PickSelectionRequest {
    ...
    llm_model: user_profile.llm_model,
    user_id: Some(user_id as i64),
    request_type: Some(crate::models::LlmRequestType::PickSelection),
  })
  .await
  {
    Ok(crate::models::LlmResponse::Stream(receiver)) => {
