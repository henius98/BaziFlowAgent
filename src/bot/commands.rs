...
pub enum Command {
  #[command(description = "✨ 输入八字 (New Profile): Create a Bazi profile; birth time and location may be skipped")]
  New,
...
pub async fn handle_command(bot: Bot, msg: Message, cmd: Command) -> ResponseResult<()> {
  ...
  if !state.runtime.allow(msg.from.as_ref().map_or(0, |user| user.id.0), state.config.runtime.requests_per_minute) {
    return Ok(());
  }
  let Some(_guard) = msg.from.as_ref().and_then(|user| crate::models::ProcessingGuard::acquire(state.clone(), user.id.0)) else {
    return Ok(());
  };
  match cmd {
    Command::New => {
    ...
    Command::Date => {
      let state = crate::models::get_state();
      let now = chrono::Utc::now().with_timezone(&state.config.app_timezone);
    ...
    Command::Schedule => {
      ...
      bot.send_message(msg.chat.id, "⏰ Select a time to receive your daily Bazi fortune reading:").reply_markup(markup).await?;
    }

    Command::ApiKey => {
      if !msg.chat.is_private() {
        bot.send_message(msg.chat.id, "Manage API keys in your private chat with the bot.").await?;
        return Ok(());
      }
      let state = crate::models::get_state();
      if let Some(user) = msg.from.as_ref() {
          ...
          None => {
            ...
            match crate::repos::create_api_key(&state.db_pool, user_id).await {
              ...
              Err(e) => {
                ...
                bot.send_message(msg.chat.id, "❌ Failed to generate API key. Please try again later.").await?;
              }
            }
          }
        }
