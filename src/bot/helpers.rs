...
///
/// Sends an initial draft, then updates at one-second intervals with a shared
/// process-wide draft budget and upstream retry-after backoff.
...
pub async fn stream_to_telegram(bot: &Bot, chat_id: ChatId, initial_text: &str, mut receiver: crate::models::LlmStream) -> String {
  ...
  let draft_id = (uuid::Uuid::now_v7().as_u128() & i64::MAX as u128) as i64 | 1;
  let client = crate::models::get_state().http_client.clone();
  ...
  if receiver.finish().await.is_err() {
    let _ = bot.send_message(chat_id, "⚠️ Analysis interrupted. Please try again.").await;
    return String::new();
  }
...
async fn flush_draft(client: &reqwest::Client, token: &str, chat_id: ChatId, draft_id: i64, text: &str, is_initial: bool) {
  ...
  if let Ok(response) = response
    ...
    && let Some(delay) = body.pointer("/parameters/retry_after").and_then(|value| value.as_u64())
  {
    state.runtime.pause_drafts(delay);
    tracing::warn!(retry_after_seconds = delay, "Telegram draft rate limited");
  }
}
