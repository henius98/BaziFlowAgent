...
pub fn init(config: &crate::config::AppConfig) -> anyhow::Result<(tracing_appender::non_blocking::WorkerGuard, tracing_appender::non_blocking::WorkerGuard)> {
  ...
  let file_appender = tracing_appender::rolling::RollingFileAppender::builder().rotation(tracing_appender::rolling::Rotation::DAILY).filename_prefix("").filename_suffix("log").build("logs")?;
  ...
  let stdout_layer = fmt::layer().with_writer(stdout_non_blocking).compact().with_target(false).with_timer(timer.clone());

  // 4. Define the file layer (no ANSI colors for the file)
  let file_layer = fmt::layer().with_ansi(false).with_writer(file_non_blocking).with_file(true).with_line_number(true).with_thread_names(true).with_timer(timer);
