...
pub async fn start_scheduler(
    ...
    user_contexts_expiration_minutes: i64,
    ...
    let cleanup_job = Job::new_async_tz(
        crate::models::get_state()
            .config
            .context_cleanup_cron
            .as_str(),
        crate::models::get_state().config.app_timezone,
        move |_uuid, _l| {
    ...
    let log_cleanup_job = Job::new_tz(
        crate::models::get_state().config.log_cleanup_cron.as_str(),
        crate::models::get_state().config.app_timezone,
        move |_uuid, _l| {
...
pub async fn add_or_update_user_schedule(bot: Bot, user_id: u64, cron_str: &str) {
    ...
    let daily_job = match Job::new_async_tz(
        cron_str,
        crate::models::get_state().config.app_timezone,
        move |_uuid, _l| {
            ...
            Box::pin(async move {
                ...
                let bazi_four_pillars = match user_profile.bazi_four_pillars {
                    ...
                    None => {
                        warn!(
                            "User {} lacks bazi profile, skipping scheduled job.",
                            user_id
                        );
                        return;
                    }
                ...
                .await
                {
                    ...
                    Err(e) => {
                        error!(
                            "Failed to fetch almanac data for user {} scheduled job: {}",
                            user_id, e
                        );
                        return;
                    }
                ...
                let bazi_summary = user_profile
                    .bazi_summary
                    .unwrap_or_else(|| user_profile.bazi_analysis.unwrap_or_default());
                ...
                .await
                {
                    Ok(crate::models::LlmResponse::Full(response)) => {
                        ...
                        for part in parts {
                            if let Err(e) =
                                bot_clone.send_message(ChatId(user_id as i64), part).await
                            {
                                error!(
                    ...
                    Ok(_) => {
                        error!(
                            "Unexpected stream response for scheduled job (user {})",
                            user_id
                        );
                    }
                    Err(e) => {
    ...
    match sched.add(daily_job).await {
        Ok(uuid) => {
            ...
            info!(
                "Successfully add scheduled job for user {} with cron {}",
                user_id, cron_str
            );
        }
        Err(e) => {
