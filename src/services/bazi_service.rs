...
pub async fn prepare_bazi_data(
    state: &std::sync::Arc<crate::models::AppState>,
    params: BaziDataParams<'_>,
) -> crate::models::AppResult<paipan::StructuredBazi> {
    ...
    let naive_dt = match chrono::NaiveDate::parse_from_str(params.birth_date, "%Y-%m-%d") {
        Ok(d) => match d.and_hms_opt(params.birth_hour as u32, params.birth_minute as u32, 0) {
            ...
            None => {
                ...
                return Err(crate::models::error::AppError::Message(
    ...
    let (structured_data, structured_json) = paipan::fetch_bazi_chart(
        &state.http_client,
        solar_dt,
        params.gender,
        birth_year,
...
pub async fn build_and_save_bazi_html(
    state: &std::sync::Arc<crate::models::AppState>,
    user_id: u64,
    username: &str,
...
pub async fn get_bazi_chart_url(
    state: &std::sync::Arc<crate::models::AppState>,
    user_id: u64,
    ...
    if let Some(bucket) = &state.r2_bucket {
        ...
        bucket
            ...
            .await
            .map_err(|e| {
                crate::models::error::AppError::Message(format!(
...
pub async fn core_bazi_analysis(
    ...
    let system_message =
            ...
            .build()
        {
            ...
            Err(_) => {
                return Err(crate::models::error::AppError::Message(
                    "Failed to build system message".to_string(),
                ));
            }
        };
    let user_message =
            ...
            .build()
        {
            ...
            Err(_) => {
                return Err(crate::models::error::AppError::Message(
                    "Failed to build user message".to_string(),
                ));
            }
        };
    ...
    match crate::services::llm::call_llm(&state.db_pool, &state.config.llm_client_config, params)
        .await
    {
        Ok(crate::models::LlmResponse::Stream(receiver)) => Ok(receiver),
        Ok(_) => Err(crate::models::error::AppError::Message(
            "Expected stream response from LLM".into(),
        )),
        Err(e) => Err(e),
    }
}
...
pub async fn generate_bazi_summary(
    ...
    let full_user_content = format!("【用户命盘详批】\n{}", bazi_analysis);
    ...
    params.user_id = Some(user_id as i64);
    params.request_type = Some("generate_bazi_summary".to_string());

    match crate::services::llm::call_llm(&state.db_pool, &state.config.llm_client_config, params)
        .await
    {
        Ok(crate::models::LlmResponse::Full(summary)) => Ok(summary),
        Ok(_) => Err(crate::models::error::AppError::Message(
            "Expected full response from LLM".into(),
        )),
        Err(e) => Err(e),
    }
}
