...
pub async fn fetch_bazi_chart(
  ...
  solar_dt: chrono::NaiveDateTime,
  gender: u8,
  birth_year: i32,
  location: Option<String>,
  birth_time_known: bool,
) -> AppResult<(StructuredBazi, String)> {
  let solar_dt_str = solar_dt.format("%Y-%m-%d %H:%M").to_string();

  let mut chart = fetch_base_bazi(client, &upstreams.chart_base, &solar_dt_str, gender).await?;
  ...
  let (yongshi, ori_relations, dy_relations, ln_relations, ln_shensha) = tokio::join!(
    fetch_yongshi(client, &upstreams.supplement_base, &solar_dt_str),
    fetch_ganzhi_relations(client, &upstreams.chart_base, &gz_str),
    fetch_ganzhi_relations(client, &upstreams.chart_base, &dy_gz_str),
    fetch_ganzhi_relations(client, &upstreams.chart_base, &ln_gz_str),
    fetch_shensha(client, &upstreams.supplement_base, &ln_gz, &gz_split_str, gender),
  );
      ...
      rels.map(|r| {
        r.into_iter()
          .filter_map(|inner_vec| {
            let filtered: Vec<_> = inner_vec.into_iter().filter(|item| !ori.iter().any(|ori_inner| ori_inner.contains(item))).collect();
  ...
  let params = MapBaziParams { chart: &chart, gender, solar_dt_str, birth_year: &birth_year, lunisolar_year: &lunisolar_year, ln_gz: &ln_gz, luck_index: &luck_index, location, birth_time_known };
...
async fn fetch_base_bazi(client: &Client, base: &str, solar_dt: &str, gender: u8) -> AppResult<RawBaziChart> {
  let api_url = format!("{base}/getbasebz8.php");
  let response = client.get(&api_url).query(&[("d", solar_dt), ("s", &gender.to_string()), ("vip", "1"), ("today", "undefined"), ("userguid", ""), ("yzs", "0")]).send().await?.error_for_status()?;
  let chart: RawBaziChart = crate::services::http::json(response).await?;
  Ok(chart)
}

async fn fetch_yongshi(client: &Client, base: &str, solar_dt: &str) -> Option<String> {
  let response = client.get(format!("{base}/getRysl.php")).query(&[("datestr", &format!("{}:00", solar_dt))]).send().await.ok()?;
  let json: Value = crate::services::http::json(response).await.ok()?;

  json.get("data").and_then(|v| v.as_str()).map(|d| d.to_string())
}

async fn fetch_ganzhi_relations(client: &Client, base: &str, gz_str: &str) -> Option<Vec<Vec<String>>> {
  let response = client.get(format!("{base}/getGZRelaction3.php")).query(&[("gz", gz_str), ("userguid", ""), ("vip", "0")]).send().await.ok()?;
  crate::services::http::json(response).await.ok()
}

// fetch shensha for dayun or liunian
async fn fetch_shensha(client: &Client, base: &str, ln: &str, gz_str: &str, gender: u8) -> Option<Vec<String>> {
  let sex_str = gender.to_string();
  let response = client
    .get(format!("{base}/getliunianshensha5.php"))
