...
impl fmt::Display for StructuredBazi {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    ...
    if self.info.birth_time_known {
      writeln!(f, "阳历:{} (排盘时间)", self.info.solar_date)?;
    } else {
      writeln!(f, "阳历:{} (出生时间未知；12:00仅为参考排盘时间)", self.info.solar_date)?;
      writeln!(f, "注意:时柱及受时柱影响的关系不可作为确定事实")?;
    }
...
pub fn generate_bazi_html(chart: &StructuredBazi, name: &str) -> String {
  // Escape all external text once before inserting it into trusted HTML markup.
  fn escape_values(value: &mut serde_json::Value) {
    match value {
      serde_json::Value::String(text) => *text = escape_html(text),
      serde_json::Value::Array(items) => items.iter_mut().for_each(escape_values),
      serde_json::Value::Object(items) => items.values_mut().for_each(escape_values),
      _ => {}
    }
  }
  let Ok(mut value) = serde_json::to_value(chart) else {
    return String::new();
  };
  escape_values(&mut value);
  let Ok(escaped) = serde_json::from_value::<StructuredBazi>(value) else {
    return String::new();
  };
  render_bazi_html(&escaped, &escape_html(name))
}
...
fn render_bazi_html(chart: &StructuredBazi, name: &str) -> String {
  ...
  let solar_date = if chart.info.birth_time_known { chart.info.solar_date.clone() } else { format!("{}（出生时间未知，12:00参考）", chart.info.solar_date) };
  html = html.replace("{{SOLAR_DATE}}", &solar_date);
  ...
  let format_relations = |items: &[String]| -> String {
    if items.is_empty() { "无明显关系".to_string() } else { items.iter().map(|r| format!("<span>{}</span>", r.split(',').next().unwrap_or(r))).collect::<Vec<_>>().join(", ") }
  };
