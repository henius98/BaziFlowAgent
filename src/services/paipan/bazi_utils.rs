...
pub fn map_bazi_data(params: MapBaziParams<'_>) -> StructuredBazi {
  ...
  let birth_time_known = params.birth_time_known;
  ...
  let mut dayun: Vec<DaYunData> = chart
    ...
    .map(|(i, (gz_str, shensha))| {
      ...
      DaYunData { is_current_dayun, start_year, end_year: start_year + 9, info: calculate_gz_info(gz_str, day_master, shensha.clone()), relation }
    })
    .collect();
  ...
  if *luck_index < dayun.len() {
    ...
    dayun = dayun.drain(luck_index.saturating_sub(1)..end).collect();
  } else {
    dayun.clear();
  }
  ...
  StructuredBazi {
    ...
    pillars,
    other: AdditionalInfo { palace_info: chart.palace_info.clone() },
...
fn extract_relations(relations: &Option<Vec<Vec<String>>>) -> Option<Relations> {
  fn clean_relations(v: &[String]) -> Vec<String> {
    v.iter().filter(|s| !s.is_empty()).map(|s| s.split(',').next().map(|first| first.to_string()).unwrap_or_else(|| s.clone())).collect()
  }
  relations
    .as_ref()
    .map(|rel| Relations { stem_relations: rel.first().map(|v| clean_relations(v)).filter(|v| !v.is_empty()), branch_relations: rel.get(1).map(|v| clean_relations(v)).filter(|v| !v.is_empty()) })
}
...
fn calculate_element_states(month_branch: &str) -> ElementStates {
  ...
  ElementStates { wood: w.to_string(), fire: f.to_string(), earth: e.to_string(), metal: m.to_string(), water: wa.to_string() }
}
...
fn arrange_liunian(chart: &RawBaziChart, lunisolar_year: &i32, ln_gz: &str) -> Option<LiuNian> {
  ...
  Some(LiuNian { year: *lunisolar_year, info: calculate_gz_info(ln_gz, day_master, shensha), relation: extract_relations(&chart.ln_gz_relations) })
}
...
pub fn get_star_luck(stem: &str, branch: &str) -> &'static str {
  ...
  let dist = if is_yin { (start_idx - branch_idx as i32).rem_euclid(12) } else { (branch_idx as i32 - start_idx).rem_euclid(12) };
...
mod boundary_tests {
  use super::*;
  #[test]
  fn first_luck_cycle_has_no_previous_cycle() {
    let mut chart: RawBaziChart = serde_json::from_str(include_str!("../../../tests/fixtures/chart.json")).unwrap();
    chart.dyshensha = vec![("甲子".into(), vec![])];
    let mapped = map_bazi_data(MapBaziParams {
      chart: &chart,
      gender: 1,
      solar_dt_str: "2026-01-01 12:00".into(),
      birth_year: &2026,
      lunisolar_year: &2026,
      ln_gz: "丙午",
      luck_index: &0,
    ...
    assert_eq!(mapped.dayun.len(), 1);
    let html = crate::services::paipan::generate_bazi_html(&mapped, "<img src=x onerror=alert(1)>");
