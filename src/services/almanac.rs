...
pub async fn fetch_and_format_almanac(client: &Client, target_date: &str) -> crate::models::AppResult<String> {
    ...
    if raw_data.is_object() {
        ...
        if let Some(day_gan_zhi) = filtered_data.get("ganZhi").and_then(|v| v.get("day")).and_then(|v| v.as_str()) {
...
fn filter_data(data: &Value, schema: &Value) -> Option<Value> {
    ...
    if let Value::Object(data_map) = data
        && let Value::Object(schema_map) = schema
    {
        let mut new_data = serde_json::Map::new();
        for (k, sub_schema) in schema_map {
            if let Some(filtered_val) = data_map.get(k).and_then(|val| filter_data(val, sub_schema)) {
...
fn calculate_kong_wang(day_gan_zhi: &str) -> String {
    ...
    let earthly_branches = ["子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥"];
...
fn translate_keys(data: Value, key_map: &HashMap<&'static str, &'static str>) -> Value {
    match data {
        ...
        Value::Array(arr) => Value::Array(arr.into_iter().map(|v| translate_keys(v, key_map)).collect()),
        other => other,
    }
}
...
fn to_plaintext(data: &Value) -> String {
    match data {
        Value::Object(map) => {
            let parts: Vec<String> = map.iter().map(|(k, v)| format!("{}: {}", k, to_plaintext(v))).collect();
            parts.join(",\n")
        }
        Value::Array(arr) => {
            let parts: Vec<String> = arr.iter().map(to_plaintext).collect();
            parts.join(" ")
        }
